...
class Voter(AbstractBaseUser, PermissionsMixin):
    ...
    # Token for WhatsApp voting link
    voting_token = models.UUIDField(default=uuid.uuid4, editable=False, unique=True, null=True, blank=True)
    voting_token_expires = models.DateTimeField(null=True, blank=True)
    voting_link_sent = models.DateTimeField(null=True, blank=True)
    ...
    def save(self, *args, **kwargs):
        if not self.pk or not self.voter_code:
            self.voter_code = self.generate_voter_code()
            # Hash the voter code like a password when first created
            self.set_password(self.voter_code)
        super().save(*args, **kwargs)

    def generate_voter_code(self):
        return uuid.uuid4().hex[:6].upper()

    def generate_voting_link(self, base_url):
        """Generate a unique voting link for this voter."""
        if not self.voting_token:
            self.voting_token = uuid.uuid4()
        # Token expires in 7 days
        self.voting_token_expires = timezone.now() + timezone.timedelta(days=7)
        self.voting_link_sent = timezone.now()
        self.save(update_fields=['voting_token', 'voting_token_expires', 'voting_link_sent'])
        return f"{base_url}/vote/{self.voting_token}/"

    def is_voting_link_valid(self):
        """Check if the voting link token is still valid."""
        if not self.voting_token or not self.voting_token_expires:
            return False
        return timezone.now() < self.voting_token_expires
